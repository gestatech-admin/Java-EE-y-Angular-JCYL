...
public class ProvinciasServicio extends BaseServicio {
	...
	public List<Provincia> getProvincias() {
		...
		CriteriaQuery<Provincia> select = query
				.where(builder.notEqual(variableRoot.get("id"), builder.parameter(Long.class, "id")));
	...
	public Object total() {
		return getSession().createCriteria(Provincia.class)
				.setProjection(Projections.count(
						getSession().getSessionFactory().getClassMetadata(Provincia.class).getIdentifierPropertyName()))
				.uniqueResult();
	}
